...
Function Register-Package {
    param(
        ...
        [Parameter(Mandatory = $false)][switch]$isType, 
        [Parameter(Mandatory = $false)][switch]$isCreator,
        [Parameter(Mandatory = $true)][int]$packageCount, 
        [Parameter(Mandatory = $false)][string]$type, 
        [Parameter(Mandatory = $false)][string]$creator, 
        [Parameter(Mandatory = $false)][string]$destination, 
        [Parameter(Mandatory = $true)][string]$logfile,
        [Parameter(Mandatory = $false)][switch]$AlreadyMoved,
        [Parameter(Mandatory = $true)][array]$packagesChecked,
        [Parameter(Mandatory = $false)][switch]$creatorFound,
        [Parameter(Mandatory = $false)][switch]$creatorNotFound,
        [Parameter(Mandatory = $false)][switch]$creatorNoType,
        [Parameter(Mandatory = $false)][switch]$return)

        if ($AlreadyMoved) {
            "Package number $packageCount : $package : This file already exists within the array. Skipping." | Out-File $logfile -Append
        } else {
            "Package number $packageCount : $package : This file has not yet been processed." | Out-File $logfile -Append
        } 
        if ($creatorNoType){
            "Package number $packageCount : $package : File matched as $creator, but we don't know the type. Moving to $destination." | Out-File $logfile -Append
            $packagesChecked += $package
            "Package number $packageCount : $package :has been processed." | Out-File $logfile -Append
        } elseif ($isType) {
            "Package number $packageCount : $package : File matched as $type. Continuing." | Out-File $logfile -Append
        } elseif ($isCreator -AND $creatorfound) {
            "Package number $packageCount : $package : File matched as $type by $creator. Moving to $destination." | Out-File $logfile -Append
            $packagesChecked += $package
            "Package number $packageCount : $package :has been processed." | Out-File $logfile -Append
        } elseif ($isCreator -AND $creatorNotFound) {
            "Package number $packageCount : $package : File matched as $type with no matching creator. Moving to $destination." | Out-File $logfile -Append
            $packagesChecked += $package
            "Package number $packageCount : $package :has been processed." | Out-File $logfile -Append
        } elseif ($return) {
            "Package number $packageCount : $package : File did not match anything. Moving to $destination." | Out-File $logfile -Append
            $packagesChecked += $package
            "Package number $packageCount : $package :has been processed." | Out-File $logfile -Append
        }
...
Function Initialize-AutoSorting {
    ...
    foreach ($package in $folderContents) {
        if ($packagesChecked -contains $package) {
            Register-Package -package $package -packageCount $packageCount -logfile $logfile -AlreadyMoved -packagesChecked $packagesChecked
            $packageCount++
            Continue
        } else {
            Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked
            $packageCount++
            ...
            if ($package.BaseName -ilike "*$recoloristWitheringSims*"){
                Register-Package -package $package -packageCount $packageCount -logfile $logfile  -packagesChecked $packagesChecked -isType -type $recoloristWitheringSims
                if ($package.Basename -ilike "*roots*" -OR $package.Basename -ilike "*hairline*") {
                    ...
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile  -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Hairline" -destination $destination
                    ...
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Hair Accessory" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*overlayacc*") { 
                    $destination = "$messyModern\Accessories\ColorOverlays\$recoloristWitheringSims"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Color Overlay" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                    ...
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Clothing" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                    ...
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Hair" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                    ...
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Shoes" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                }  elseif ($package.Basename -ilike "*acc*") {
                    $destination = "$messyModern\Accessories\$recoloristWitheringSims"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristWitheringSims -type "Accessories" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } else {
                    $destination = "$manualSort\$recoloristWitheringSims"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -destination $destination -creatorNoType -creator $recoloristWitheringSims
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                }
            } elseif ($package.BaseName -ilike "*jewl*"){ 
                Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isType -type $recoloristJewl
                if ($package.Basename -ilike "*roots*" -OR $package.Basename -ilike "*hairline*") {
                    $destination = "$messyGeneral\Hairlines\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Hairline" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.Basename -ilike "*dyeaccessory*" -OR $package.BaseName -ilike "*headband*" -OR $package.BaseName -ilike "*hairband*" -OR $package.BaseName -ilike "*scrunchie*" -OR $package.BaseName -ilike "*clips*" -OR $package.BaseName -ilike "*ombre*" -OR $package.BaseName -ilike "*hairbow*" -OR $package.BaseName -ilike "*hairclips*") {
                    $destination = "$messyGeneral\HairAccessories\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Hair Accessory" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*overlayacc*") { 
                    $destination = "$messyModern\Accessories\ColorOverlays\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Color Overlay" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*top*" -OR $package.BaseName -ilike "*bottom*" -OR $package.BaseName -ilike "*bodysuit*" -OR $package.BaseName -ilike "*jeans*" -OR $package.BaseName -ilike "*outfit*" -OR $package.BaseName -ilike "*pants*" -OR $package.BaseName -ilike "*skirt*" -OR $package.BaseName -ilike "*sweater*" -OR $package.BaseName -ilike "*shirt*") {
                    $destination = "$messyModern\Clothing\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Clothing" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*hair*" -OR $package.BaseName -ilike "*hairstyle*" -OR $package.BaseName -ilike "*twintails*" -OR $package.BaseName -ilike "*ponytail*" -OR $package.BaseName -ilike "*braid*" -OR $package.BaseName -ilike "*hairstyle*") {
                    $destination = "$messyGeneral\HairRecolors\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Hair" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*sneakers*" -OR $package.BaseName -ilike "*shoes*") {
                    $destination = "$messyModern\Shoes\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Shoes" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                }  elseif ($package.Basename -ilike "*acc*") {
                    $destination = "$messyModern\Accessories\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristJewl -type "Accessories" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } else {
                    $destination = "$manualSort\$recoloristJewl"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -destination $destination -creatorNoType -creator $recoloristJewl
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                }
            } elseif ($package.BaseName -ilike "*candynaturals*"){ 
                Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isType -type $recoloristCandy
                if ($package.Basename -ilike "*roots*" -OR $package.Basename -ilike "*hairline*") {
                    $destination = "$messyGeneral\Hairlines\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Hairline" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.Basename -ilike "*dyeaccessory*" -OR $package.BaseName -ilike "*headband*" -OR $package.BaseName -ilike "*hairband*" -OR $package.BaseName -ilike "*scrunchie*" -OR $package.BaseName -ilike "*clips*" -OR $package.BaseName -ilike "*ombre*" -OR $package.BaseName -ilike "*hairbow*" -OR $package.BaseName -ilike "*hairclips*") {
                    $destination = "$messyGeneral\HairAccessories\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Hair Accessory" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*overlayacc*") { 
                    $destination = "$messyModern\Accessories\ColorOverlays\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Color Overlay" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*top*" -OR $package.BaseName -ilike "*bottom*" -OR $package.BaseName -ilike "*bodysuit*" -OR $package.BaseName -ilike "*jeans*" -OR $package.BaseName -ilike "*outfit*" -OR $package.BaseName -ilike "*pants*" -OR $package.BaseName -ilike "*skirt*" -OR $package.BaseName -ilike "*sweater*" -OR $package.BaseName -ilike "*shirt*") {
                    $destination = "$messyModern\Clothing\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Clothing" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*hair*" -OR $package.BaseName -ilike "*hairstyle*" -OR $package.BaseName -ilike "*twintails*" -OR $package.BaseName -ilike "*ponytail*" -OR $package.BaseName -ilike "*braid*" -OR $package.BaseName -ilike "*hairstyle*") {
                    $destination = "$messyGeneral\HairRecolors\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Hair" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } elseif ($package.BaseName -ilike "*sneakers*" -OR $package.BaseName -ilike "*shoes*") {
                    $destination = "$messyModern\Shoes\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Shoes" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                }  elseif ($package.Basename -ilike "*acc*") {
                    $destination = "$messyModern\Accessories\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -creator $recoloristCandy -type "Accessories" -destination $destination
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                } else {
                    $destination = "$manualSort\$recoloristCandy"
                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -destination $destination -creatorNoType -creator $recoloristCandy
                    New-Item -ItemType Directory -Force -Path $destination
                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                    Continue
                }
                ...
                Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -isCreator -destination $destination -creatorNoType -creator "LorySims"
                New-Item -ItemType Directory -Force -Path $destination
                Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                Continue
            } else {
                        ...
                        if ($package.BaseName -ilike "*$typeCheck*") { #discover the type
                            ...
                            for ($creatorsCount=0; $creators.Count -gt $creatorsCount; $creatorsCount++) {
                                ...
                                if ($package.BaseName -ilike "*$creatorCheck*") { #match to a creator
                                    ...
                                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -return -destination $destination -type $typeCheck -isCreator -creator $creatorCheck
                                    New-Item -ItemType Directory -Force -Path $destination
                                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                                    Continue
                                } elseif ($creatorsLength -le $creatorsCount) {
                                    $destination = $mainFolder
                                    Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -return -destination $destination -type $typeCheck -isCreator -creatorNotFound
                                    New-Item -ItemType Directory -Force -Path $destination
                                    Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                                    Continue
                                }
                            ...
                            $destination = "$messyFolder\Manual Sort"
                            Register-Package -package $package -packageCount $packageCount -logfile $logfile -packagesChecked $packagesChecked -return -destination $destination
                            New-Item -ItemType Directory -Force -Path $destination
                            Move-Item <#-Verbose-#> -Path $($package.FullName) -Destination "$destination\$($package.Name)"
                            Continue
                        }
...
"simmerianne93"
"simplesimmer"
"simmireenxherecirmxsimmerianne93"
"sims3melancholic"
"sims41ife"
"simsberrry"
"simstomaggie"
...
$typesList = @(<#--- -1 ---#> "brooch"
    ...
    $typeFolder = @(<#--- -1 ---#> "$messyGeneral\Accessories\Brooches"
    <#--- 0 ---#> "$periodManualSort"
    <#--- 1 ---#> "$period10s"
    <#--- 2 ---#> "$period10s"
    <#--- 3 ---#> "$period10s"
...
Initialize-AutoSorting -messyfolder $messyfolder -creators $creatorsList -typeOfCC $typesList -folderForType $typeFolder
